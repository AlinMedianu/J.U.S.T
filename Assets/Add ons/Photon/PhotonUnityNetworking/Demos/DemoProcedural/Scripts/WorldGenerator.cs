...
namespace Photon.Pun.Demo.Procedural
{
    ...
    public class WorldGenerator : MonoBehaviour
    {
        ...
        private Dictionary<int, UnityEngine.GameObject> clusterList;
        ...
        public void Awake()
        {
            clusterList = new Dictionary<int, UnityEngine.GameObject>();
        ...
        {
            foreach (UnityEngine.GameObject cluster in clusterList.Values)
            {
        ...
        private IEnumerator GenerateWorld()
        {
            ...
            for (int x = 0; x < (int) WorldSize; x += (int) Mathf.Sqrt((int) ClusterSize))
            {
                ...
                {
                    UnityEngine.GameObject cluster = new UnityEngine.GameObject();
            ...
            foreach (UnityEngine.GameObject cluster in clusterList.Values)
            {
                ...
                for (int x = 0; x < (int)Mathf.Sqrt((int)ClusterSize); ++x)
                {
                    for (int z = 0; z < (int)Mathf.Sqrt((int)ClusterSize); ++z)
                    {
                        ...
                        UnityEngine.GameObject block = UnityEngine.GameObject.CreatePrimitive(PrimitiveType.Cube);
            ...
            foreach (DictionaryEntry entry in PhotonNetwork.CurrentRoom.CustomProperties)
            {
                ...
                UnityEngine.GameObject cluster;
